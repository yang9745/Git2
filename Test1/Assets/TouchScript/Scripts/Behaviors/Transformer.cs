        ...
        private void stateChangedHandler(object sender, GestureStateChangeEventArgs gestureStateChangeEventArgs)
        {
            switch (gestureStateChangeEventArgs.State)
            {
                ...
                case Gesture.GestureState.Possible:
                    stateManual();                
                    //Debug.Log("鼠标1:" + GetComponent<TransformGesture>().getPointPreviousScreenPosition(0));
                    //Camera.main.GetComponent<CameraController>().GetRay(GetComponent<TransformGesture>().getPointPreviousScreenPosition(0));
                    //Debug.Log("鼠标2" + GetComponent<TransformGesture>().getPointScreenPosition(0));
                    //Debug.Log("鼠标" + cachedTransform.position);                 
                    break;
